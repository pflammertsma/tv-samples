    ...
    Surface(
        ...
        TextField(
            ...
            onValueChange = { newValue ->
                Log.i("TvTextField", "onValueChange called: old='$value', new='$newValue'")
                onValueChange(newValue)
            },
                    ...
                    when (event.key) {
                        Key.Enter, Key.NumPadEnter -> {
                            if (event.type == KeyEventType.KeyDown) {
                                Log.i("TvTextField", "Consuming KeyDown for Enter to prevent IME character insertion")
                            } else if (event.type == KeyEventType.KeyUp) {
                                Log.i("TvTextField", "Executing IME action on KeyUp for Enter")
                                if (onImeAction != null) {
                                    onImeAction()
                                } else {
            ...
            keyboardActions = KeyboardActions(
                onAny = {
                    Log.i("TvTextField", "keyboardActions.onAny triggered for imeAction=$imeAction")
                    if (onImeAction != null) {
                        onImeAction()
                    } else {
                        focusManager.moveFocus(FocusDirection.Down)
